...
const Base = () => {
  ...
  const submitHandler1 = async (event) => {
    ...
    console.log(body)
    const response = await create('/createBase', body, token)
    if(response){
  ...
  const submitHandler2 = async (event) => {
    ...
    const response = await create('/addBase', body, token)
    if(response){
  ...
  const fetchData = async () => {
    const response = await fetchPizza("/getAllBase");
  ...
  return (
    <div className='w-[90%] h-[86vh] text-white text-center rounded-xl mt-2 p-5 space-y-4 text-xl flex-col items-center justify-center bg-gray-600'>
      ...
      <div className='overflow-auto'>
        <p className='text-2xl text-yellow-300 underline mb-3'>Base Stock</p>
        {
          baseData.map((item) => (
